...
Template.complete_list.onCreated(async function () {
...
Template.complete_list.helpers({
  settings: function () {
    return {
      collection: 'batchlistcomplete',
      rowsPerPage: 20,
      showFilter: false,
      fields: [
        ...
        {
          label: 'Action', tmpl: Template.comdetail, sortable: false,
        ...
        },
        {
          ...
          cellClass: function (data) {
            if (!data) {
              return 'badge badge-secondary mr-1'
            }
            else if (data == 'accept') {
...
Template.comdetail.helpers({
  showcomplete: function () {
    var sh = ReactiveMethod.call('showcomplete', this._id);
    if (sh) {
      $('#com' + this._id).prop('hidden', true);
    }
  },
})
Template.comdetail.events({
  ...
  'click .btnCompleted': function (e, tpl) {
    e.preventDefault();
    var id = this._id;
    $('#com' + this._id).prop('hidden', true);
    Meteor.call("updateCompleted", id, function (error, res) {
      if (res) {
        toastr.success('PickList is Completed');
        // console.log('success update accept list')
      } else {
        toastr.success('PickList is Failed to Completed');
