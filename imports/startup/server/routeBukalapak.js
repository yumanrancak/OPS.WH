...
import { BukalapakOrders, BukalapakOrderDetail, ProductOrders, allOrder } from '../../api/orders/orders';
...
JsonRoutes.add('POST', 'orders/bukalapak/insert', function (request, response) {
    ...
    if (checkOrder == 0) {
        ...
        console.log('Bukalapak insertId: ' + insertId);
        // console.log('bukalapak result:' + result);
        productData.forEach(product => {
            var productId = ProductOrders.insert(product);
            console.log('Bukalapak productId: ' + productId);
        });
        // //new order
        ...
        console.log('Bukalapak duplicate: ' + order_id);
        JsonRoutes.sendResult(response, {
            code: 200,
            data: {
...
JsonRoutes.add('POST', 'orders/bukalapak/UpdateOrder', function (request, response) {
    ...
    if (checkOrder > 0) {
        ...
        console.log('Bukalapak Update Data: ' + order_id);
    } else {
        // console.log('Null')
        JsonRoutes.sendResult(response, {
            code: 200,
        ...
        console.log('Bukalapak Data Not Found, fetch new order: ' + order_id);
        Meteor.call('getNewOrder', order_id, 'bukalapak');
    }
});