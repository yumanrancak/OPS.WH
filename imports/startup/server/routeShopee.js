...
import { ShopeeOrders, ShopeeStatus, ShopeeTracking, ProductOrders, allOrder } from '../../api/orders/orders';
...
JsonRoutes.add('POST', 'orders/shopee/insert', function (request, response) {
    ...
    if (checkOrder == 0) {
        ...
        console.log('Shopee insertId: ' + insertId);
        // console.log('shopee result:' + result);
        productData.forEach(product => {
            var productId = ProductOrders.insert(product)
            console.log('Shopee productId: ' + productId);
        });
        //new order
        ...
        console.log('Shopee duplicate: ' + order_id);
        JsonRoutes.sendResult(response, {
            code: 200,
            data: {
...
JsonRoutes.add('POST', 'orders/shopee/UpdateOrder', function (request, response) {
    ...
    if (checkOrder > 0) {
        ...
        console.log('Shopee Update Data: ' + order_id);
    } else {
        // console.log('Null')
        JsonRoutes.sendResult(response, {
            code: 200,
        ...
        console.log('Shopee Data Not Found, fetch new order: ' + order_id);
        Meteor.call('getNewOrder', order_id, 'shopee');
    }
});