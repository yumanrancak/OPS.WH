...
import { ZaloraOrderDetail, ZaloraOrders, ProductOrders, allOrder } from '../../api/orders/orders';
...
JsonRoutes.add('POST', 'orders/zalora/insert', function (request, response) {
    ...
    if (checkOrder == 0) {
        ...
        console.log('Zalora insertId: ' + insertId);
        productData.forEach(product => {
            var productId = ProductOrders.insert(product);
            console.log('Zalora productId: ' + productId);
        });
        ...
        console.log('Zalora duplicate: ' + order_id);
        JsonRoutes.sendResult(response, {
            code: 200,
            data: {
...
JsonRoutes.add('POST', 'orders/zalora/UpdateOrder', function (request, response) {
    ...
    if (checkOrder > 0) {
        ...
        console.log('Zalora Update Data: ' + order_id);
    } else {
        // console.log('Null')
        JsonRoutes.sendResult(response, {
            code: 200,
        ...
        console.log('Zalora Data Not Found, fetch new order: ' + order_id);
        Meteor.call('getNewOrder', order_id, 'zalora');
    }
});