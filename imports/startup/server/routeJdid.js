...
import { allOrder, JdidOrderDetail, JdidOrders, ProductOrders } from '../../api/orders/orders';
...
JsonRoutes.add('POST', 'orders/jdid/insert', function (request, response) {
    ...
    if (checkOrder == 0) {
        ...
        console.log('JDid insertId: ' + insertId);
        // console.log('jdid result:' + result);
        productData.forEach(product => {
            var productId = ProductOrders.insert(product)
            console.log('JDid productId: ' + productId);
        });
        ...
        console.log('JDid duplicate: ' + order_id);
        JsonRoutes.sendResult(response, {
            code: 200,
            data: {
...
JsonRoutes.add('POST', 'orders/jdid/UpdateOrder', function (request, response) {
    // console.log(request.body)jdid
    let result = request.body
    let order_id = result.order_id;
    ...
    if (checkOrder > 0) {
        ...
        console.log('JDid Update Data: ' + order_id);
    } else {
        // console.log('Null')
        JsonRoutes.sendResult(response, {
            code: 200,
        ...
        console.log('JDid Data Not Found, fetch new order: ' + order_id);
        Meteor.call('getNewOrder', order_id, 'jdid');
    }
});